...
- Only suggest a specialist when:
  ...
  b) You've had at least 3-4 exchanges and have gathered sufficient information to determine that specialized care is needed
...
"""
        ...
        for chunk in stream:
            if chunk.choices[0].delta.content:
                ...
                # 🚀 EXTRACT AND STREAM RESPONSE TEXT
                if not response_started and '"response"' in complete_response:
                    response_started = True
                    # Find the start of the response text
                    start_idx = complete_response.find('"response":"') + len('"response":"')
                    if start_idx > len('"response":"') - 1:
                        response_text = complete_response[start_idx:]

                if response_started:
                    # Extract new response text from this chunk
                    if '"response":"' in content:
                        # This chunk contains the start of response
                        start_idx = content.find('"response":"') + len('"response":"')
                        new_text = content[start_idx:]
                    else:
                        # Check if we're still in the response field
                        if '","' not in content and '"}' not in content:
                            new_text = content
                        else:
                            # We've reached the end of response field
                            end_idx = min(
                                content.find('","') if '","' in content else len(content),
                                content.find('"}') if '"}' in content else len(content)
                            )
                            new_text = content[:end_idx]
                            response_started = False  # Stop streaming

                    # Clean and send new text
                    if new_text and new_text not in ['","', '"}', '"']:
                        # Clean escape characters
                        clean_text = new_text.replace('\\"', '"').replace('\\n', '\n')

                        if not first_chunk_sent:
                            logger.info(f"🌊 First text chunk: '{clean_text[:30]}...'")
                        ...
                        # 🎵 Add to streaming audio manager
                        await audio_manager.add_text_chunk(clean_text, websocket)
        ...
        await websocket.send_text(json.dumps({
            ...
            "metadata": {
                "extracted_keywords": response_json["extracted_keywords"],
                "suggested_specialist": response_json.get("suggested_specialist"),
                "refer_to_general": response_json.get("refer_to_general"),
                "refer_to_specialist": response_json.get("refer_to_specialist")
            }
        }))
    ...
    try:
        ...
        while True:
            ...
            if "text" in data:
                ...
                # 🚀 OPTIMIZATION: Add to in-memory chat history immediately
                ...
                if current_persona != "general" and response_data.get("refer_to_specialist"):
                    ...
                    if target_specialist in ["psychologist", "dietician"] and target_specialist != current_persona:
                        ...
                        # Generate new response from target specialist
                ...
                # Check if specialist wants to refer back to General OPD
                    ...
                    # Generate new response from General OPD
                    response_data = generate_ai_response(
                ...
                # 🚀 OPTIMIZATION: Parallel audio generation and database save
                response_id = str(uuid.uuid4())
                ...
                # 🚀 OPTIMIZATION: Start audio generation and database save in parallel
                ...
                logger.info(f"🎵 Audio generated in {audio_time:.2f}s")

                # Don't wait for database - let it complete in background
                # db_success = db_future.result()

                # Calculate total request time
                total_time = time.time() - request_start
                logger.info(f"🏁 Total request processed in {total_time:.2f}s (AI: {ai_time:.2f}s, Audio: {audio_time:.2f}s)")

                # Send response
                await websocket.send_text(json.dumps({
                    "response": response_data["response"],
                    "extracted_keywords": response_data["extracted_keywords"],
                    "suggested_specialist": response_data.get("suggested_specialist", "none") if current_persona == "general" else None,
                    "audio": audio_data,
                    "response_id": response_id,
                    "current_persona": current_persona,
                    "processing_time": f"{total_time:.2f}s"  # Add timing info for debugging
                }))
            ...
            # Handle audio input
                ...
                try:
                        ...
                        for msg in reversed(chat_history[-5:]):
                            if msg["role"] == "assistant" and "suggested_specialist" in msg:
                                ...
                                if suggested in ["psychologist", "dietician"]:
                                    ...
                                    # Generate new response from specialist
                                    response_data = generate_ai_response(
                                        f"Hello, I'm a patient who was just transferred to you, First Intrduce Yourself. My original question was: {user_input}",
                                        patient_info,
                                        chat_history,
                                        system_prompt,
                                        retriever
                    ...
                    # 🚀 OPTIMIZATION: Add AI response to in-memory chat history immediately
                    chat_history.append({"role": "assistant", "content": response_data["response"]})

                    # 🚀 OPTIMIZATION: Generate audio and save to database in parallel
                    response_id = str(uuid.uuid4())
                    ...
                    # Start audio generation and database save in parallel
                    audio_start = time.time()
                    audio_future = thread_pool.submit(generate_audio, response_data["response"], voice)
                    db_future = thread_pool.submit(save_ai_response_to_database)
                    ...
                    logger.info(f"🏁 Total audio request processed in {total_audio_time:.2f}s (Transcription: {transcription_time:.2f}s, AI: {ai_time:.2f}s, Audio: {audio_time:.2f}s)")
                    ...
                    await websocket.send_text(json.dumps({
                        ...
                        "response_id": response_id,
                        "current_persona": current_persona,
                        "processing_time": f"{total_audio_time:.2f}s"  # Add timing info for debugging
                    }))
