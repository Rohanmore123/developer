    ...
    try:
        ...
        while not authenticated:
            try:
                ...
                # Get chat history
                chat_history = get_chat_history(patient_id, db=db)
        ...
        while True:
            ...
            if "text" in data:
                ...
                # Update chat history (this is fast and can be done in the main thread)
                chat_history.append({"role": "user", "content": user_input})

                # We don't need to wait for the database operation to complete
                # It will run in the background while we continue processing

                # Select appropriate persona and retriever
                ...
                # Start parallel tasks for audio generation and database operations
                def save_to_database():
                    try:
                        # Create a new session for this thread
                        with SessionLocal() as thread_db:
                            # Set timestamp, createdAt, and
                            now = datetime.now()
                            thread_db.add(ChatMessage(
                ...
                db_future = thread_pool.submit(save_to_database)
                ...
                # If database operation failed, log it but continue
                if not db_success:
                    logger.warning("Database operation failed, but continuing with response")
            ...
            # Handle audio input
                ...
                try:
                    ...
                    # Generate audio
                    audio_data = generate_audio(response_data["response"], voice)
